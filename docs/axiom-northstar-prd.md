...
A development team using Axiom:

1. **Writes specs, not code.** Every system behavior is defined in Axiom's spec language — a blend of natural language intent, typed input/output examples, property-based invariants, performance bounds, and failure-mode declarations.
...
4. **Runs CI that regenerates and verifies.** On every commit, the Axiom runtime regenerates code from the spec and runs the full verification suite. If regeneration produces code that fails any assertion, the commit is blocked.
...
6. **Migrates trivially.** Language migration = change the compilation target in the Axiom config. Framework upgrade = regenerate. The spec doesn't change.
...
### 2.3 What Axiom Is Not

- **Not a code generation copilot.** Copilots assist within the code-as-source paradigm. Axiom replaces it.
- **Not a testing framework.** Tests verify code. Axiom specs *define* behavior; code is a byproduct.
...
```
┌──────────────┐     ┌──────────────────┐     ┌─────────────────┐
│  Spec Change  │────▶│  Axiom Compiler   │────▶│  LLM Generation │
...
Axiom must not be coupled to a single LLM provider.
...
- **Fine-tuned fallback**: As the spec corpus grows, Axiom trains specialized models on (spec → verified code) pairs. These become the default for common patterns, reducing cost and latency.
...
```
cache_key = hash(spec_content + target_language + model_version + axiom_runtime_version)
```
...
Axiom assigns every spec a **completeness score** based on:
...
2. **Replay against spec.** Axiom replays the input against the spec's declared behavior. If the spec says the behavior is correct, the spec is wrong. If the spec says the behavior is wrong, the regeneration failed verification (shouldn't have shipped — this is a harness bug).
3. **Gap analysis.** Axiom presents: "Your spec does not define behavior for this input class. Add an example or invariant?" The developer adds to the spec, regenerates, and verifies.
4. **Code explanation.** For deep debugging, Axiom can "explain" the generated code — mapping code blocks back to the spec clauses they satisfy. This is the "decompiler" for spec-driven development.
...
Hand-written code is a first-class citizen, not a second-class workaround. The interface between spec-driven modules and hand-written modules is a typed contract enforced by Axiom.
...
# Axiom does NOT regenerate this code.
# Axiom DOES verify it against these invariants on every CI run.
# Axiom DOES enforce the typed interface at composition boundaries.
```
...
- Migration path: when a hand-written module's behavior stabilizes, it can be converted to a spec + regenerated code. Axiom provides tooling to draft the initial spec from the existing code.
...
|-----------|-------------|--------------|
| **Axiom Spec Language** | The spec format + parser + LSP (language server protocol for IDE support) | Open source |
| **Axiom CLI** | Local regeneration + verification for individual developers | Open source |
| **Axiom Runtime** | The regeneration engine (LLM orchestration, caching, dependency resolution) | Open source core, hosted premium |
| **Axiom CI** | Hosted regeneration + verification as a CI/CD service (GitHub Actions integration, etc.) | SaaS — per-seat + compute usage |
| **Axiom Cache** | Hosted deterministic cache with team sharing, audit trail, and rollback | SaaS — included in CI tier |
| **Axiom Dashboard** | Spec completeness scoring, regeneration analytics, drift detection, team metrics | SaaS — included in CI tier |
| **Axiom IDE Extension** | VS Code / JetBrains extension with spec authoring, inline verification, and code preview | Free (drives CI adoption) |
...
- Publish the "Axiom Manifesto" — the philosophical argument for specs-as-source.
...
- Position as: "You're already spending $X on modernization. Axiom makes modernization a one-time spec-writing effort, not an ongoing maintenance burden."
...
| Player | What They Do | How Axiom Differs |
|--------|-------------|-------------------|
| **GitHub Copilot** | AI-assisted code completion within code-as-source paradigm | Axiom replaces the paradigm, not assists within it |
| **Cursor** | AI-native IDE, still code-as-source | Same — smarter editing of an artifact Axiom eliminates |
| **Devin / Codegen agents** | Autonomous coding agents that write code | Agents produce code nobody understands; Axiom produces specs everybody understands |
...
| **Gherkin / Cucumber** | Behavior-driven development (BDD) | Specs are decorative (tests verify code); in Axiom, specs ARE the source |
| **Terraform / Pulumi** | Infrastructure as code (declarative specs → infra) | Same mental model, but for infrastructure. Axiom applies it to application code |

### 8.2 Incumbent Risk

GitHub (Microsoft), Anthropic, Google, or Cursor could add spec-driven features. Defenses:

1. **Spec corpus moat.** Once teams write specs in Axiom format, switching costs are high.
...
4. **Philosophical commitment.** Incumbents profit from code complexity (more code = more Copilot seats). Axiom profits from eliminating it. Misaligned incentives slow incumbents.
...
Axiom is built using Axiom. The percentage of Axiom's own codebase that is spec-driven is a public metric and a core credibility signal.
...
- **Dogfood percentage** of Axiom's own codebase
...
6. **What's the git workflow?** (Specs in `/specs`, generated code in `.axiom-cache/`, `.gitignore` the cache? Or cache in a separate branch?)
...
- **Property-based testing (QuickCheck/Hypothesis)** — verification approach Axiom adopts
- **Behavior-Driven Development (Cucumber/Gherkin)** — spec format Axiom supersedes
